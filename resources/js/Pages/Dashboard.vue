...
<template>
    <AppLayout title="Dashboard">
        <template #header>
            <h2 v-if="$page.props.active.roles?.includes('manager')"
                class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                Sales
            </h2>
            <h2 v-else="$page.props.active.roles?.includes('admin')"
                class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                Dashboard
            </h2>
        </template>
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div class="flex-grow p-2 min-w-full sm:min-w-0 sm:flex-auto md:flex-1">
                        <div
                            ...
                            <div class="text-center">
                                ...
                                <p class="text-3xl font-bold text-blue-500">{{ orders }}</p>
                            </div>
                        </div>
                    </div>
                    <div class="flex-grow p-2 min-w-full sm:min-w-0 sm:flex-auto md:flex-1">
                ...
                <div v-else="$page.props.active.roles?.includes('admin')"
                    class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                    <Welcome />
                </div>
                <div v-if="$page.props.active.roles?.includes('manager')" class="flex justify-center items-center shadow border-b border-gray-200 sm:rounded-lg mt-6">
                    <table class="w-full">
                        <!-- Table Header -->
                        ...
                        <tbody class="dark:bg-gray-700 divide-y text-white divide-gray-200 ">
                            <tr v-for="order in orders_today" :key="order.id">
                                ...
                                <td class="px-6 py-4 text-center whitespace-nowrap">{{ order.address }}</td>
